...
# External Manager : count
with open('./results/kpsa_preprocess.txt', 'w', encoding='utf-8') as fout:
    for sent in dataset:
        fout.write(sent + "\n")
fout.close()
...
co_occur = ptm.cooccurrence.CooccurrenceExternalManager(
    ...
    input_file='../results/kpsa_preprocess.txt',
    output_file='../results/kpsa_co_count.txt',
...
co_results = []
vocabulary = {}
with open('./results/kpsa_co_count.txt', 'r', encoding='utf-8') as fin:
    for line in fin:
        fields = line.split()
